...
def _sign(body: bytes) -> str:
    """HMAC-SHA256 signature of the request body, keyed by WEBHOOK_SECRET."""
...
def send_req(w_address: str, data):
    try:
        ...
        if WEBHOOK_SECRET:
            # Keep the legacy plaintext header for backward compatibility and add
            # a tamper-proof HMAC signature receivers can verify.
            req_headers["x-webhook-secret"] = WEBHOOK_SECRET
            req_headers["x-webhook-signature"] = f"sha256={_sign(body)}"
        r = session.post(w_address, data=body, headers=req_headers)
        if r.ok:
            return True
        logger.error(r)
...
if WEBHOOK_ADDRESS:
    ...
    scheduler.add_job(run_if_leader(send_notifications), "interval",
                      seconds=JOB_SEND_NOTIFICATIONS_INTERVAL,
                      replace_existing=True)
    scheduler.add_job(run_if_leader(delete_expired_reminders), "interval", hours=2,
