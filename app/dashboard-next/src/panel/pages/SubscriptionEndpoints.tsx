...
const SubscriptionEndpointsInner: FC = () => {
  ...
  return (
    <div className="sk-sub-ep">
      <div className="sk-sub-ep-hero">
        <div>
          <h3 className="sk-sub-ep-hero-title">{t("subEndpoints.hintTitle")}</h3>
          <p className="sk-sub-ep-hero-body">{t("subEndpoints.hintBodyCompact")}</p>
        </div>
        <div className="sk-sub-ep-hero-meta" dir="ltr">
          <span>{filtered.length}</span>
          <small>{t("subEndpoints.panelCount")}</small>
        </div>
      </div>

      <div className="sk-sub-ep-toolbar">
        <Input
          dir="ltr"
          placeholder={t("subEndpoints.searchPlaceholder")}
          value={q}
          onChange={(e) => setQ(e.target.value)}
          className="sk-sub-ep-search"
        />
      ...
      {loading ? (
        ...
        <div className="sk-sub-ep-grid-cards">
          {filtered.map((g) => {
            const primary = groupPrimary(g);
            ...
            return (
              <article key={g.key} className="sk-sub-ep-card">
                <header className="sk-sub-ep-card-head">
                  <div>
                    <div className="sk-sub-ep-card-title" dir="ltr">
                      {g.label}
                    </div>
                    <div className="sk-sub-ep-card-host" dir="ltr">
                      {host}
                      {port != null ? `:${port}` : ""}
                    </div>
                  </div>
                  <span className={`sk-sub-ep-pill ${allOn ? "is-on" : "is-off"}`}>
                    {allOn ? t("common.enabled") : t("common.disabled")}
                  </span>
                </header>

                <div className="sk-sub-ep-channels">
                  {present.map(({ id, ep }) => (
                    <button
                      key={id}
                      type="button"
                      className="sk-sub-ep-chip"
                      dir="ltr"
                      onClick={() => setEditing({ group: g, tab: id })}
                      title={t("subEndpoints.editChannel", { kind: t(`subEndpoints.kind.${id}`) })}
                    >
                      <span className="sk-sub-ep-chip-kind">{t(`subEndpoints.kind.${id}`)}</span>
                      <span className="sk-sub-ep-chip-path">/{ep!.path_prefix}/</span>
                      {!ep!.enabled && <span className="sk-sub-ep-chip-off">off</span>}
                    </button>
                  ))}
                  {g.extras.map((ep) => (
                    <span key={ep.id} className="sk-sub-ep-chip is-static" dir="ltr">
                      <span className="sk-sub-ep-chip-kind">{ep.slug}</span>
                      <span className="sk-sub-ep-chip-path">/{ep.path_prefix}/</span>
                    </span>
                  ))}
                </div>

                <footer className="sk-sub-ep-card-foot">
                  <span className="sk-faint" style={{ fontSize: 12 }}>
