...
setup_tls() {
  if [ "${SKIP_HTTPS}" = "1" ]; then
    warn "SKIP_HTTPS=1 — leaving the panel on http://127.0.0.1:${PANEL_PORT} (set up a proxy yourself)."
    return 0
  fi
...
load_env_creds() {
  ...
  export ADMIN_USERNAME PANEL_PORT
}
...
write_env() {
  ...
  local admin_user admin_pass hash public_ip
...
# Panel binds to localhost; nginx terminates TLS (scripts/setup_https.sh).
UVICORN_HOST=127.0.0.1
UVICORN_PORT=${PANEL_PORT}
ALLOWED_ORIGINS=http://127.0.0.1:${PANEL_PORT}
...
print_access() {
  ...
  echo
  echo "  ${BOLD}Dashboard${NC}  ${base_url}/dashboard/"
  echo "  ${BOLD}System${NC}     ${base_url}/dashboard/#/system"
  case "$base_url" in
    https://*) :;;
  ...
  if [ -n "${ADMIN_USERNAME:-}" ] && [ -n "${ADMIN_PASSWORD:-}" ]; then
    ...
    echo
    echo "  ${YELLOW}Admin username: ${ADMIN_USERNAME:-admin} (see ${APP_DIR}/.env).${NC}"
...
cmd_doctor() {
  ...
  echo
  echo "  Open in browser: ${BOLD}${base_url}/dashboard/${NC}"
