#!/usr/bin/env bash
# Shahkar host management console.
#
# Invoked on the panel HOST as `shahkar`. Drives the docker-compose deployment
# (or a systemd deployment as a fallback) and proxies admin management to the
# in-container `shahkar-cli`. Bilingual (English / فارسی). Every destructive
...
DATA_DIR="${SHAHKAR_DATA_DIR:-/var/lib/shahkar}"
COMPOSE_PROJECT="${COMPOSE_PROJECT_NAME:-shahkar}"
SERVICE="${SHAHKAR_COMPOSE_SERVICE:-shahkar}"  # compose service name AND systemd unit name
GIT_BRANCH_DEFAULT="master"
LANG_FILE="${SHAHKAR_LANG_FILE:-$DATA_DIR/.shahkar-lang}"
SHAHKAR_LANG="en"
...
if [ -t 1 ]; then
  ...
  C_WHITE="\033[0;37m"
  # Brand palette (matches dashboard --sk-accent / --sk-accent-2)
...
# ── Shahkar brand UI primitives (NOT x-ui boxed clone) ──────────────────────
...
EOF
  printf "%b" "$C_RESET"
  printf "  %bSHAHKAR%b  %bhost console%b  %b·%b  %b%s%b  %b·%b  %bv%s%b
" \
    "${C_BOLD}${C_BRAND}" "$C_RESET" \
    "$C_MUTED" "$C_RESET" \
    "$C_MUTED" "$C_RESET" \
    "$C_INDIGO" "$SHAHKAR_LANG" "$C_RESET" \
    "$C_MUTED" "$C_RESET" \
...
EN[auto_no_container]="Panel container not found — is the panel running? Try: shahkar start"
FA[auto_no_container]="کانتینر پنل پیدا نشد — پنل بالا است؟ اول: shahkar start"
...
EN[need_root]="This action needs root. Re-run with: sudo shahkar"
FA[need_root]="این عملیات به دسترسی root نیاز دارد. با sudo shahkar اجرا کنید"
EN[done]="Done."
FA[done]="انجام شد."
EN[aborted]="Aborted."
FA[aborted]="لغو شد."
...
EN[ssl_missing]="SSL helper not found. Run: shahkar https"
FA[ssl_missing]="ابزار SSL یافت نشد. اجرا کنید: shahkar https"
...
EN[not_installed]="Shahkar directory not found"
FA[not_installed]="پوشهٔ Shahkar یافت نشد"
EN[uninstall_warn]="This will STOP and REMOVE the Shahkar containers."
FA[uninstall_warn]="این کار کانتینرهای Shahkar را متوقف و حذف می‌کند."
EN[uninstall_keep]="Your data in /var/lib/shahkar and code are kept unless you choose otherwise."
FA[uninstall_keep]="داده‌های شما در /var/lib/shahkar و کد نگه داشته می‌شوند مگر خلافش را انتخاب کنید."
EN[uninstall_data]="Also DELETE all panel DATA in /var/lib/shahkar? (IRREVERSIBLE)"
FA[uninstall_data]="همچنین همهٔ داده‌های پنل در /var/lib/shahkar حذف شود؟ (غیرقابل‌بازگشت)"
...
EN[uninstall_bin]="Remove the 'shahkar' command itself from this host?"
FA[uninstall_bin]="خود دستور 'shahkar' هم از این سرور حذف شود؟"

t() { local k="$1" v=""; if [ "$SHAHKAR_LANG" = "fa" ]; then v="${FA[$k]:-}"; fi; [ -z "$v" ] && v="${EN[$k]:-$k}"; printf '%s' "$v"; }

load_lang() {
  [ -f "$LANG_FILE" ] && SHAHKAR_LANG="$(tr -d '[:space:]' < "$LANG_FILE" 2>/dev/null)"
  case "$SHAHKAR_LANG" in fa|en) ;; *) SHAHKAR_LANG="en" ;; esac
}
save_lang() {
...
panel_container() {
  ...
  if [ -z "$cid" ]; then
    cid="$(docker ps -aq --filter "name=shahkar-shahkar" 2>/dev/null | head -n1 | tr -d '[:space:]')"
  fi
  if [ -z "$cid" ]; then
    cid="$(docker ps -aq --filter "name=shahkar-shahkar" 2>/dev/null | head -n1 | tr -d '[:space:]')"
  fi
...
write_autostart_override() { # $1 = always|no
  ...
  cat > "$ov" <<EOF
# Managed by \`shahkar\` Autostart (options 15/16). Safe to delete to reset.
services:
  ${SERVICE}:
    restart: "${policy}"
EOF
...
panel_cli()     { case "$DEPLOY_MODE" in docker) dc exec -T "$SERVICE" shahkar-cli "$@";; systemd) ( cd "$APP_DIR" && python3 shahkar-cli.py "$@" );; *) return 1;; esac; }
panel_cli_tty() { case "$DEPLOY_MODE" in docker) dc exec "$SERVICE" shahkar-cli "$@";; systemd) ( cd "$APP_DIR" && python3 shahkar-cli.py "$@" );; *) return 1;; esac; }
...
owner_bcrypt() { # $1 = plaintext
  local pass="$1"
  local py='import os;from passlib.hash import bcrypt;print(bcrypt.using(rounds=12).hash(os.environ["SHAHKAR_ADMIN_PASSWORD"]))'
  case "$DEPLOY_MODE" in
    docker)  dc exec -T -e "SHAHKAR_ADMIN_PASSWORD=$pass" "$SERVICE" python3 -c "$py" 2>/dev/null | tr -d '\r\n' ;;
    systemd) ( cd "$APP_DIR" && SHAHKAR_ADMIN_PASSWORD="$pass" python3 -c "$py" ) 2>/dev/null | tr -d '\r\n' ;;
    *) return 1 ;;
  esac
}
...
action_install() {
  ...
  if command -v shahkar >/dev/null 2>&1; then shahkar install; else err "$(t not_installed): shahkar manager missing"; fi
}
...
action_update() {
  require_root || return 1
  if command -v shahkar >/dev/null 2>&1; then
    shahkar update; return $?
  fi
...
action_update_menu() {
  ...
  if [ -f "$APP_DIR/scripts/install-shahkar.sh" ]; then
    bash "$APP_DIR/scripts/install-shahkar.sh"
  fi
  ok "$(t done)"
  exec "$0" "$@"
}
...
reset_db_admin_via_env() { # $1=password(may be empty) $2=new username(may be empty)
  ...
  case "$DEPLOY_MODE" in
    docker)  dc exec -T -e "SHAHKAR_ADMIN_PASSWORD=$pass" "$SERVICE" shahkar-cli admin reset-credentials "${extra[@]}" ;;
    systemd) ( cd "$APP_DIR" && SHAHKAR_ADMIN_PASSWORD="$pass" python3 shahkar-cli.py admin reset-credentials "${extra[@]}" ) ;;
    *) return 1 ;;
  esac
}
...
action_ssl() {
  require_root || return 1
  if command -v shahkar >/dev/null 2>&1; then msg "$(t ssl_delegate)"; shahkar https; return $?; fi
...
action_bbr() {
  ...
  local f=/etc/sysctl.d/99-shahkar-bbr.conf
  {
...
action_geo() {
  require_root || return 1
  msg "$(t geo_updating)"
  local tmp; tmp="$(mktemp -d /tmp/shahkar-geo.XXXXXX)" || { err "$(t geo_fail)"; return 1; }
...
action_uninstall() {
  ...
  if confirm "$(t uninstall_bin)"; then rm -f /usr/local/bin/shahkar /usr/bin/shahkar 2>/dev/null || true; ok "$(t done)"; fi
  exit 0
}
...
# --------------------------------------------------------------------------
# Menu — Shahkar brand identity (teal hub, node sections, left rail)
# --------------------------------------------------------------------------
print_footer() {
...
usage() {
  cat <<EOF
shahkar — Shahkar host management console

Usage:
  shahkar                 Open the interactive menu (bilingual EN/FA)
  shahkar status | start | stop | restart | restart-xray
  shahkar logs            Follow panel logs
  shahkar update          Pull latest + rebuild + restart
  shahkar autostart on|off|status
  shahkar password        Reset an admin username/password
  shahkar settings        View current settings
  shahkar ssl             Set up / refresh HTTPS
  shahkar firewall        Manage UFW
  shahkar bbr             Enable BBR
  shahkar geo             Update Xray geo files
  shahkar create-admin | admins
  shahkar lang <en|fa>    Set menu language
  shahkar uninstall
  shahkar help
EOF
}
      ...
      autostart|auto)
        case "${2:-}" in
          ...
          status|"")
            ...
            ;;
          *) err "Usage: shahkar autostart on|off|status"; exit 1 ;;
        esac
        ;;
