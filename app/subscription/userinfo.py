...
def format_subscription_profile_title(user: "UserResponse") -> str:
    """Human-readable subscription group title shown in client apps."""
    if not SUB_PROFILE_TITLE_DYNAMIC:
        return SUB_PROFILE_TITLE

    parts: list[str] = []
    brand = (SUB_PROFILE_TITLE or "").strip()
    if brand and brand.lower() not in {user.username.lower(), "subscription"}:
        parts.append(brand)
...
def subscription_client_import_url(url: str, user: "UserResponse") -> str:
    ...
    if base and not base.endswith("/"):
        base = f"{base}/"
    title = format_subscription_profile_title(user)
...
def build_subscription_body_preamble(
    ...
    *,
    ...
    lines = [
        ...
        f"{prefix}profile-title: {encode_title(format_subscription_profile_title(user))}",
    ...
    if SUB_SUPPORT_URL:
        lines.append(f"{prefix}support-url: {SUB_SUPPORT_URL}")
    return "\n".join(lines) + "\n"
...
def attach_subscription_body_metadata(
    ...
    *,
    ...
    preamble = build_subscription_body_preamble(
        user, config_format, profile_web_page_url=profile_web_page_url
    )
    if not preamble:
        return body
