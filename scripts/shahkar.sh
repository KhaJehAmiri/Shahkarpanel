#!/usr/bin/env bash
#
# Shahkar installer & manager (branded host console).
#
# One-line install (run as root):
#   bash /opt/shahkar/scripts/shahkar.sh install
#
# Interactive shell installer (Shahkar-branded menus in this terminal).
# Browser wizard: WEB_WIZARD=1 install
# Plain CLI:       SKIP_WIZARD=1 install
#
# After install, manage with:
#   shahkar                 interactive host console
#   shahkar <command>  install | update | up | down | restart | status | logs
...
EOF
  printf "%b" "$NC"
  echo "  ${BOLD}${BRAND}SHAHKAR${NC}  ${MUTED}installing…${NC}"
  echo "  ${MUTED}──────────────────────────────────────────────${NC}"
  echo
}
...
emit("PANEL_TITLE", cfg.get("panel_title") or "Shahkar")
...
EOF
  printf "%b" "$NC"
  echo "  ${BOLD}${BRAND}SHAHKAR${NC}  ${MUTED}web installer${NC}"
  echo "  ${MUTED}──────────────────────────────────────────────${NC}"
...
start_shell_installer() {
  ...
  log "Starting Shahkar installer wizard…"
  chmod +x "$wiz"
...
start_tui_installer() {
  ...
  rm -f "${INSTALL_CONFIG_FILE}"
  write_progress "waiting" 0 "Waiting for installer configuration…" false
  log "Starting Shahkar terminal installer…"
...
start_install_wizard() {
  ...
  if [ ! -f "$wizard_py" ]; then
    ...
    die "Web installer files not found after git clone. Run: shahkar update && shahkar install"
  fi
  if [ "${SKIP_WIZARD}" = "1" ]; then
    prompt_install_config
    return 1
  ...
  rm -f "${INSTALL_CONFIG_FILE}"
  write_progress "waiting" 0 "Waiting for installer configuration…" false
  log "Starting Shahkar web installer on port ${INSTALLER_PORT}…"
  python3 "$wizard_py" \
    --port "${INSTALLER_PORT}" \
  ...
  if command -v ufw >/dev/null 2>&1; then
    ufw allow "${INSTALLER_PORT}/tcp" comment 'Shahkar installer (temporary)' >/dev/null 2>&1 || true
  fi
...
prepare_low_memory() {
  ...
  if [ "${SKIP_NODE_BUILD:-}" = "" ]; then
    SKIP_NODE_BUILD=1
    log "Skipping node-agent build during install (build later: docker build -t shahkar/node:latest ${APP_DIR}/node)"
  fi
}
...
compose() {
  ...
  # Autostart toggle (shahkar 15/16) writes this; -f disables automatic merge.
  if [ -f "${APP_DIR}/docker-compose.override.yml" ]; then
    file_args+=(-f "${APP_DIR}/docker-compose.override.yml")
  fi
  if docker compose version >/dev/null 2>&1; then
    docker compose -p shahkar "${file_args[@]}" "${env_args[@]}" "$@"
  else
    docker-compose -p shahkar "${file_args[@]}" "${env_args[@]}" "$@"
  fi
}
...
disable_conflicting_services() {
  if systemctl is-enabled shahkar.service >/dev/null 2>&1; then
    warn "Disabling host systemd unit (shahkar.service) — Docker owns the panel."
    systemctl stop shahkar.service >/dev/null 2>&1 || true
    systemctl disable shahkar.service >/dev/null 2>&1 || true
  fi
}
...
configure_firewall() {
  ...
  ufw allow 443/tcp comment 'Shahkar HTTPS' >/dev/null 2>&1 || true
  for p in ${XRAY_INBOUND_PORTS}; do
    ufw allow "${p}" comment 'Shahkar Xray inbound' >/dev/null 2>&1 || true
  done
...
setup_tls() {
  ...
  if DOMAIN="${DOMAIN}" EMAIL="${EMAIL}" PANEL_PORT="${PANEL_PORT}" bash "$script" "${args[@]}"; then
    ...
    warn "Automatic HTTPS setup did not complete. Re-run later: shahkar https"
...
load_env_creds() {
  ...
  # a re-run / `shahkar info` can still show the operator their real password
...
# Generated by shahkar installer on $(date -u +%FT%TZ)
...
EOF
  ...
  cat >> "${RUNTIME_ENV_FILE}" <<EOF
POSTGRES_USER=shahkar
POSTGRES_PASSWORD=${pg_pass}
POSTGRES_DB=shahkar
SQLALCHEMY_DATABASE_URL=postgresql://shahkar:${pg_pass}@127.0.0.1:5432/shahkar
...
XRAY_JSON=/var/lib/shahkar/xray_config.json
# Private paths so the non-root panel user can upgrade/downgrade Xray in-dashboard.
XRAY_EXECUTABLE_PATH=/var/lib/shahkar/bin/xray
XRAY_ASSETS_PATH=/var/lib/shahkar/share/xray

# Panel defaults (set by web installer).
PANEL_DEFAULT_LANG=${PANEL_DEFAULT_LANG:-en}
PANEL_TITLE=${PANEL_TITLE:-Shahkar}
...
# Backups
BACKUP_DIR=/var/lib/shahkar/backups
...
EOF
  ...
  mkdir -p "${DATA_DIR}"
  cat > "$cred_file" <<CREDS
# Shahkar install credentials — $(date -u +%FT%TZ)
...
Database       : PostgreSQL shahkar@127.0.0.1:5432/shahkar
CREDS
  chmod 600 "$cred_file"
...
install_cli() {
  log "Installing host command (shahkar)…"
  if [ -f "${APP_DIR}/scripts/shahkar.sh" ]; then
    cp "${APP_DIR}/scripts/shahkar.sh" "${BIN_PATH}"
  ...
  # Interactive management console (`shahkar`).
  if [ -f "${APP_DIR}/scripts/install-shahkar.sh" ]; then
    bash "${APP_DIR}/scripts/install-shahkar.sh" >/dev/null 2>&1 || true
  elif [ -f "${APP_DIR}/scripts/shahkar-manager.sh" ]; then
    cat > /usr/local/bin/shahkar <<EOF
#!/usr/bin/env bash
export SHAHKAR_APP_DIR="${APP_DIR}"
export COMPOSE_PROJECT_NAME="\${COMPOSE_PROJECT_NAME:-shahkar}"
exec bash "${APP_DIR}/scripts/shahkar-manager.sh" "\$@"
EOF
    chmod +x /usr/local/bin/shahkar
  fi
  # Keep installer available as shahkar-install for rare recovery use.
  if [ -f "${APP_DIR}/scripts/shahkar.sh" ] && [ ! -e /usr/local/bin/shahkar-install ]; then
    cp "${APP_DIR}/scripts/shahkar.sh" /usr/local/bin/shahkar-install
    chmod +x /usr/local/bin/shahkar-install
  fi
  ok "Command ready: shahkar"
}
...
build_node_image() {
  ...
  log "Building shahkar/node agent image (SSH provisioning)..."
  if docker build -t shahkar/node:latest "${APP_DIR}/node"; then
    ok "Node image: shahkar/node:latest"
  else
    warn "Node image build failed. Build manually: docker build -t shahkar/node:latest ${APP_DIR}/node"
  fi
}
...
wait_for_panel() {
  ...
  for i in $(seq 1 150); do
    ...
    if [ $((i % 15)) -eq 0 ]; then
      ...
      compose logs --tail=8 shahkar 2>/dev/null | tail -5 || true
    fi
    sleep 2
  done
  warn "Panel API did not respond in time. Check: shahkar logs"
  compose logs --tail=40 shahkar 2>/dev/null || true
}
...
apply_install_branding() {
  ...
  PANEL_TITLE="${PANEL_TITLE}" PRIMARY_COLOR="${PRIMARY_COLOR:-#2ee0c4}" SUPPORT_URL="${SUPPORT_URL:-}" \
    compose exec -T -e PANEL_TITLE -e PRIMARY_COLOR -e SUPPORT_URL shahkar python3 <<'PY' 2>/dev/null || true
import os
from app.db import GetDB
from app.tenant import set_branding
title = os.environ.get("PANEL_TITLE", "Shahkar")
...
EOF
  printf "%b" "$NC"
  echo "  ${BOLD}${BRAND}SHAHKAR${NC}  ${MUTED}setup${NC}"
  echo "  ${MUTED}──────────────────────────────────────────────${NC}"
  echo
  prepare_low_memory
  install_deps
  ...
  if [ "${SKIP_NODE_BUILD:-0}" = "1" ]; then
    warn "SKIP_NODE_BUILD=1 — node image skipped (build later: docker build -t shahkar/node:latest ${APP_DIR}/node)"
  else
...
print_access() {
  ...
  elif compose ps --status running 2>/dev/null | grep -q shahkar; then
    panel_txt="container up, API down (run: shahkar logs)"; panel_col="$YELLOW"
  else
    panel_txt="not running (run: shahkar up)"; panel_col="$BADC"
  fi
  if docker image inspect shahkar/node:latest >/dev/null 2>&1; then
    node_txt="built"; node_col="$OKC"
  else
    node_txt="missing (docker build -t shahkar/node:latest node/)"; node_col="$YELLOW"
  fi
  db_txt="PostgreSQL · ${POSTGRES_USER:-shahkar}@127.0.0.1:5432/${POSTGRES_DB:-shahkar}"

  echo
  box_top
  box_head "◆  SHAHKAR — installation complete"
  box_sep
  box_head "ACCESS"
  ...
  case "$base_url" in
    https://*) :;;
    *) box_note "! HTTPS not enabled yet — run: shahkar https" "$YELLOW";;
  esac
  box_sep
  ...
  if [ -f "${DATA_DIR}/install-credentials.txt" ]; then
    ...
    box_note "Forgot it? Set a new one: shahkar password" "$MUTED"
  fi
  box_sep
  box_head "SYSTEM"
  ...
  box_sep
  box_head "MANAGE"
  box_kv "shahkar" "host console (status · update · SSL · password)" "$MUTED"
  box_kv "shahkar-cli" "check · info · logs · update · backup" "$MUTED"
...
cmd_info() {
  need_root
  [ -d "${APP_DIR}" ] || die "Shahkar not installed in ${APP_DIR}"
  print_access
}
...
cmd_doctor() {
  ...
  echo "${BOLD}Shahkar diagnostics${NC}"
  echo
  ...
  if [ "$code" = "200" ]; then ok "Local ${url}${DASHBOARD_PATH:-/dashboard/} → HTTP $code"; else
    err "Local ${url}${DASHBOARD_PATH:-/dashboard/} → HTTP $code (expected 200). Check: shahkar logs"
  fi
  ...
  if systemctl is-active nginx >/dev/null 2>&1; then
    ...
    warn "nginx not active — HTTPS not terminated. Run: shahkar https"
  fi
...
repair_nginx_stub_mounts() {
  ...
  [ -f "$script" ] || return 0
  bash "$script" || warn "nginx stub-mount repair reported issues (subscription domains may need: shahkar https)"
}
...
cmd_https() {
  ...
  [ -d "${APP_DIR}" ] || die "Shahkar not installed in ${APP_DIR}"
  local script="${APP_DIR}/scripts/setup_https.sh"
  [ -f "$script" ] || die "scripts/setup_https.sh not found. Run: shahkar update"
  load_env_creds
  local args=(--port "${PANEL_PORT}")
  [ -n "${DOMAIN}" ] && args+=(--domain "${DOMAIN}")
  [ -n "${EMAIL}" ]  && args+=(--email "${EMAIL}")
...
sync_agent_github_if_configured() {
  ...
  if NODE_AGENT_IMAGE="${NODE_AGENT_IMAGE:-shahkar/node:latest}" \
...
sync_agent_mirror_if_configured() {
  ...
  if NODE_AGENT_MIRROR_SSH="${NODE_AGENT_MIRROR_SSH}" \
     ...
     NODE_AGENT_IMAGE="${NODE_AGENT_IMAGE:-shahkar/node:latest}" \
     APP_DIR="${APP_DIR}" \
     bash "$script"; then
...
cmd_cli()     { need_root; compose exec -T ${APP_NAME} shahkar-cli "$@"; }
...
cmd_restore() {
  need_root
  local file="${1:-}"; [ -n "$file" ] && [ -f "$file" ] || die "Usage: shahkar restore <backup.tar.gz>"
...
cmd_uninstall() {
  need_root
  read -r -p "${YELLOW}Remove Shahkar and ALL data? [y/N]: ${NC}" ans
  case "${ans:-N}" in
    y|Y)
...
usage() {
  cat <<EOF
${BOLD}${BRAND}SHAHKAR${NC}  ${MUTED}host manager${NC}
...
Usage: ${APP_NAME} <command>
  ...
  cli <args...>      Run shahkar-cli inside the container
  uninstall          Remove the app (keeps data dir)

Use: ${BOLD}shahkar${NC}  — interactive management console (status, SSL, geo, admins, …)
EOF
}
